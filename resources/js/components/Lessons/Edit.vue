  ...
  <div class="bg-bg text-text transition-colors duration-300">
    <div class="max-w-4xl mx-auto px-4 py-8">
      ...
      <div class="mb-8 text-center">
        ...
        <p class="text-lg opacity-75">Update your lesson details</p>
      </div>
      ...
      <div class="bg-card border border-border rounded-2xl shadow-lg overflow-hidden">
        <form @submit.prevent="submitForm" enctype="multipart/form-data" class="p-6 space-y-6">
          ...
          <div>
            ...
            <!-- Current File -->
            <div v-if="currentFileUrl" class="mb-4 p-4 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-800">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                  <i class="fas fa-file text-green-500 text-xl"></i>
                  <div>
                    <p class="font-semibold text-green-800 dark:text-green-300">Current File</p>
            ...
            <div class="border-2 border-dashed border-border rounded-xl p-6 text-center transition-all duration-300 hover:border-blue-400 hover:bg-blue-50/50 group">
              ...
              <label class="cursor-pointer">
                ...
                <div class="space-y-2">
                  <p class="text-lg font-semibold">Upload New File</p>
...
const removeCurrentFileFlag = ref(false)
...
const currentFileUrl = computed(() => {
  return props.lesson.file_url && !removeCurrentFileFlag.value ? props.lesson.file_url : null
})

const currentFileName = computed(() => {
  if (!props.lesson.file_url) return ''
  return props.lesson.file_url.split('/').pop() || 'Download File'
})
...
const removeCurrentFile = () => {
  removeCurrentFileFlag.value = true
  toast.info('Current file will be removed after saving')
}
...
const submitForm = async () => {
  ...
  try {
    ...
    if (removeCurrentFileFlag.value) {
      formData.append('remove_file', '1')
    }

    const response = await axios.post(props.updateRoute, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      }
    })

    toast.success('Lesson updated successfully!', { timeout: 3000 })
    setTimeout(() => {
    ...
    if (err.response?.status === 422) {
      ...
      toast.error(err.response?.data?.message || 'Failed to update lesson. Please try again.', { timeout: 4000 })
    }
  } finally {
...
onMounted(() => {
  ...
  form.duration = props.lesson.duration || 30
  form.is_free = props.lesson.is_free ? '1' : '0'
  validateTitle()
})
</script>