  ...
  <div class="bg-bg text-text transition-colors duration-300">
    <div class="max-w-4xl mx-auto px-4 py-8">
      ...
      <div class="mb-8 text-center">
        ...
        <p class="text-lg opacity-75">Update student information</p>
      </div>
...
const submitForm = async () => {
  ...
  try {
    ...
    if (data.success) {
      toast.success('Student updated successfully!', { timeout: 3000 })
      setTimeout(() => { window.location.href = props.indexRoute }, 1200)
    }
  } catch (err) {
    console.error(err.response?.data || err)
    toast.error('Failed to update student. Try again.', { timeout: 4000 })
  } finally {
